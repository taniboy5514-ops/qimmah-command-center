...
function miniReport(agent, cycleNo, highlights, directive) {
  ...
  if (directive) text += " Executing CEO directive: \"" + String(directive).slice(0, 100) + "\".";
...
export async function runSquadCycle(S, up, log, onPhase) {
  ...
  /* Live fleet activity — timestamped lines land in the Live Feed as each
     phase happens, so the user sees the 60 agents working in real time. */
  const pushAct = (entries) => {
    try { up((s) => ({ ...s, feed: [...entries, ...(s.feed || [])].slice(0, 100) })); } catch (e) { /* activity lines are best-effort */ }
  };

  // Phase 1 — 60 agents report (local, instant, free)
  setPhase("agents");
  const highlights = (S.results || []).filter((r) => r.topic).slice(0, 6).map((r) => r.topic);
  const currentDirs = S.squadDirectives || {};
  const reports = AGENTS.map((a) => miniReport(a, cycleNo, highlights, currentDirs[a.squad] && currentDirs[a.squad].text));
  {
    const sample = [reports[7], reports[23], reports[38]].filter(Boolean);
    pushAct(sample.map((r) => ({ id: uid(), type: "agent", text: r.code + " · " + r.name + " filed report (" + r.status + ")", ts: Date.now(), by: "Squad " + r.squad }))
      .concat([{ id: uid(), type: "autopilot", text: "Squad cycle " + cycleId + ": all 60 agents filed mini-reports", ts: Date.now(), by: "Squad Cycle" }]));
  }
  ...
  for (const sq of SQUADS) {
    ...
    pushAct([{ id: uid(), type: "agent", text: "Squad " + sq + " Alpha compiled digest (" + reports.filter((r) => r.squad === sq).length + " reports)" + (digests[sq].offline ? " [offline]" : ""), ts: Date.now(), by: "Squad " + sq }]);
  }

  // Phase 3 — CEO Brain produces the consolidated Full Study
  setPhase("ceo");
  pushAct([{ id: uid(), type: "study", text: "CEO Brain studying the 5 squad digests…", ts: Date.now(), by: "CEO Brain" }]);
...
/* Compact "Fleet activity" ticker — latest agent/autopilot/study lines from
   the Live Feed, so the Agents view visibly moves while cycles run. */
export function FleetActivity({ S }) {
  const items = (S.feed || []).filter((f) => f.type === "agent" || f.type === "autopilot" || f.type === "study").slice(0, 8);
  if (!items.length) return null;
  return (
    <Card style={{ marginBottom: 16, padding: 14 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
        <span className="q-blink" style={{ width: 8, height: 8, borderRadius: "50%", background: "#34D399", boxShadow: "0 0 8px #34D399" }} />
        <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", color: "#C4B5FD" }}>Fleet activity · live</div>
        <span style={{ fontSize: 10.5, color: "#6B6685", marginLeft: "auto", textTransform: "uppercase", letterSpacing: 1 }}>full log in Live Feed</span>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {items.map((f) => (
          <div key={f.id} style={{ display: "flex", alignItems: "baseline", gap: 10, fontSize: 12.5 }}>
            <span style={{ color: "#6B6685", fontSize: 10.5, textTransform: "uppercase", letterSpacing: 1, flexShrink: 0, width: 62 }}>{timeAgo(f.ts)}</span>
            <span style={{ color: "#D8D3E8", lineHeight: 1.5 }}>{f.text}</span>
          </div>
        ))}
      </div>
    </Card>
