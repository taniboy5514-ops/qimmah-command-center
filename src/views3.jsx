...
/* ============================================================
   MCP EXECUTION LOG — live viewer for the tool_executions table.
   Polls /api/mcp/executions every 10s while visible. Honest-limits
   style: when the backend or the table is missing, the panel shows
   a plain note instead of fake rows.
   ============================================================ */
export function McpExecutionLog() {
  const [rows, setRows] = useState(null); // null = still loading
  const [fail, setFail] = useState("");   // endpoint reported ok:false or is unreachable
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        const res = await fetch("/api/mcp/executions");
        if (!alive) return;
        if (res.ok) {
          const json = await res.json();
          if (json.ok === false) { setFail(json.error || "The execution log is unavailable."); setRows([]); }
          else { setRows(json.executions || []); setFail(""); }
        } else {
          setFail("The execution log endpoint returned " + res.status + ".");
          setRows([]);
        }
      } catch (e) {
        if (alive) { setFail("The MCP backend is not reachable from here — the execution log lights up on the deployed app."); setRows([]); }
      }
    }
    poll();
    ...
    <Card style={{ marginBottom: 18 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, marginBottom: 10, flexWrap: "wrap" }}>
        <div style={{ fontSize: 11, letterSpacing: 2.5, textTransform: "uppercase", color: CYAN, fontWeight: 600, display: "flex", alignItems: "center", gap: 7 }}>
          <Activity size={13} />
          MCP Execution Log
          <span className="q-blink" style={{ width: 7, height: 7, borderRadius: "50%", background: fail ? "#FBBF24" : "#34D399", boxShadow: "0 0 8px " + (fail ? "#FBBF24" : "#34D399") }} />
        </div>
        {rows && !fail && <span style={{ fontSize: 11, color: "#8B86A3" }}>latest {rows.length} · refreshes every 10s</span>}
      </div>
      ...
      {!fail && rows === null && (
        <div style={{ fontSize: 12.5, color: "#8B86A3" }}>Loading executions…</div>
      )}

      {!fail && rows && rows.length === 0 && (
        <div style={{ fontSize: 12.5, color: "#8B86A3", lineHeight: 1.6 }}>
          No tool executions recorded yet. When the agent fleet runs MCP tools (approvals, goals, squad cycles), every call lands here with its latency and cost.
        </div>
      )}
      ...
      {!fail && rows && rows.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 6, maxHeight: 420, overflowY: "auto" }}>
          {rows.map((r) => {
            ...
            return (
              <div key={r.id} style={{ ...glass, padding: "8px 10px" }}>
                <button onClick={() => setOpenId(open ? null : r.id)}
                  style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", background: "none", border: "none", cursor: "pointer", padding: 0, textAlign: "left", fontFamily: "inherit", flexWrap: "wrap" }}>
                  {open ? <ChevronDown size={13} style={{ color: "#6B6685", flexShrink: 0 }} /> : <ChevronRight size={13} style={{ color: "#6B6685", flexShrink: 0 }} />}
                ...
                {open && (
                  <div style={{ marginTop: 8, paddingTop: 8, borderTop: "1px solid rgba(255,255,255,0.07)" }}>
                    {r.error && <div style={{ fontSize: 12, color: "#FCA5A5", marginBottom: 6, whiteSpace: "pre-wrap" }}>{String(r.error)}</div>}
                    <div style={{ fontSize: 10, letterSpacing: 1, textTransform: "uppercase", color: "#8B86A3", marginBottom: 4 }}>args</div>
                    <pre style={{ margin: 0, fontSize: 11, color: "#C9C4DC", whiteSpace: "pre-wrap", wordBreak: "break-all", fontFamily: "monospace" }}>{argsPreview(r.args)}</pre>
                  </div>
                )}
              </div>
            );
