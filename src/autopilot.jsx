...
export const CURRICULUM = [
  ...
  "Same-day delivery workflows — how the 60-agent fleet ships a website in hours, not days",
  "Oman lead-generation tactics that work this quarter — WhatsApp, Instagram, referrals",
  "Value-based pricing in OMR — when to charge 99, 400 or 500+ per month",
  "Client communication speed — response-time habits that keep Omani clients for years",
  "The Qimmah tool stack — free and cheap tools the fleet uses to work fast",
  "Objection handling for Omani SME owners — scripts that close on WhatsApp",
  "Scope control — delivering fast without scope creep on OMR 400 websites",
  "Retention playbooks — monthly check-ins that stop retainer churn",
];
...
export function resultMarkdown(r) {
  ...
  if (r.digests) {
    md += "## Squad digests\n";
    Object.entries(r.digests).forEach(([sq, d]) => {
      md += "### Squad " + sq + "\n- Wins: " + d.wins + "\n- Blockers: " + d.blockers + "\n- Metrics: " + d.metrics + "\n- Asks: " + d.asks + "\n\n";
    });
  }
        ...
        <div style={{ marginTop: 10 }}>
          ...
          {r.directives && (
            <div style={{ marginBottom: 10 }}>
              ...
              <div style={{ fontSize: 12.5, color: "#D8D3E8", lineHeight: 1.7, display: "flex", flexDirection: "column", gap: 6 }}>
                {Object.entries(r.directives).map(([sq, d]) => {
                  const sc = SQUAD_META[sq] ? SQUAD_META[sq].color : "#A78BFA";
                  return (
                    <div key={sq} style={{ padding: "8px 10px", borderRadius: 8, background: sc + "11", border: "1px solid " + sc + "33" }}>
                      <span style={{ fontWeight: 700, color: sc }}>Squad {sq}: </span>{d}
                    </div>
                  );
                })}
              </div>
          ...
          {r.crossInsights && r.crossInsights.length > 0 && (
            <div style={{ marginBottom: 10 }}>
              <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", color: "#22D3EE", marginBottom: 5 }}>Cross-squad insights</div>
              <div style={{ fontSize: 12.5, color: "#D8D3E8", lineHeight: 1.7 }}>{r.crossInsights.map((p, i) => <div key={i}>{"\u2022"} {p}</div>)}</div>
            </div>
          )}
          {r.digests && (
            <div style={{ marginBottom: 10 }}>
              <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", color: "#C4B5FD", marginBottom: 5 }}>Squad digests</div>
              <div style={{ fontSize: 12, color: "#B7B2CC", lineHeight: 1.6, display: "flex", flexDirection: "column", gap: 6 }}>
                {Object.entries(r.digests).map(([sq, d]) => (
                  <div key={sq}>
                    <b style={{ color: (SQUAD_META[sq] || {}).color || "#A78BFA" }}>Squad {sq}</b> — Wins: {d.wins} · Blockers: {d.blockers} · Metrics: {d.metrics} · Asks: {d.asks}
                  </div>
                ))}
              </div>
            </div>
